...
import { SUMMARY_QUEUE_CONFIG } from '@/constants/queue'
  ...
  try {
    worker = new Worker<SummaryJob>(
      ...
      },
      {
        ...
        concurrency: SUMMARY_QUEUE_CONFIG.CONCURRENCY,
        limiter: {
          max: SUMMARY_QUEUE_CONFIG.RATE_LIMITER.MAX_JOBS,
          duration: SUMMARY_QUEUE_CONFIG.RATE_LIMITER.DURATION_MS,
        },
      }
    )
