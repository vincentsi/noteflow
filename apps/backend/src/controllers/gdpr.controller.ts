...
export class GDPRController {
  ...
  exportUserData = asyncHandler(async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    return reply.status(200).send(SuccessResponse.ok({
   ...
   */
  deleteUserData = asyncHandler(async (request: FastifyRequest<{
    Body: {
      confirmEmail: string
      reason?: string
    }
    ...
    if (confirmEmail !== userEmail) {
      ...
      return reply.status(400).send(ErrorResponse.badRequest('Email confirmation does not match. Please provide your correct email address.'))
    }
   ...
   */
  anonymizeUserData = asyncHandler(async (request: FastifyRequest<{
    Body: {
      confirmEmail: string
    }
  }>, reply: FastifyReply) => {
